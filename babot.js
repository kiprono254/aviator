...
async function getRandomGiveawayWinner(chat) {
  try {
    ...
    const eligibleParticipants = participants.filter(participant => {
      const isValid = participant.id && participant.id._serialized && 
                     participant.id._serialized.includes('@c.us');
      if (!isValid) {
        console.log('⚠️ Filtered invalid participant:', participant.id);
      }
      return isValid;
    });
        ...
        if (chat.isGroup) {
          try {
            ...
            if (winner) {
              ...
              await message.reply('❌ Could not find any eligible participants for the giveaway. The group might be empty or there may be a sync issue.');
            }
          } catch (error) {
      ...
      if (message.body === '.tagall') {
        if (chat.isGroup) {
          try {
            ...
            if (!participants || participants.length === 0) {
              await message.reply('❌ No participants found in this group. This might be a sync issue with WhatsApp.');
              return;
            }
            ...
            for (const participant of participants) {
              if (participant.id && participant.id._serialized && participant.id.user) {
            ...
            if (mentions.length === 0) {
              await message.reply('❌ No valid participants to tag. Participant data may be incomplete.');
              return;
            }
