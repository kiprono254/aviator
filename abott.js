...
async function getRandomGiveawayWinner(chat) {
  try {
    ...
    const eligibleParticipants = participants.filter(participant => {
      return !participant.id._serialized.includes('@c.us') || 
             (participant.id._serialized.includes('@c.us') && 
              !participant.id._serialized.includes('status'));
    });
    ...
    console.log('Error getting giveaway winner:', error);
    return null;
  }
}
        ...
        if (chat.isGroup) {
          try {
            ...
            if (winner) {
              ...
              await message.reply('❌ Could not find any eligible participants for the giveaway.');
            }
          } catch (error) {
            console.log('Error in giveaway command:', error);
            await message.reply('❌ Error executing giveaway command. Make sure I have admin permissions to read participants.');
          }
        } else {
...
Use .listwords to see all forbidden words`;
      ...
      if (message.body === '.tagall') {
        if (chat.isGroup) {
          try {
            const participants = chat.participants;
            const mentions = participants.map(participant => `@${participant.id.user}`).join(' ');
            await chat.sendMessage(`📢 Attention all members! ${mentions}`, {
              mentions: participants
            });
          } catch (error) {
            await message.reply('❌ Error executing command. Make sure I have admin permissions.');
          }
        } else {
          await message.reply('❌ This command only works in groups.');
        }
