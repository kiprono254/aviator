...
async function getRandomGiveawayWinner(chat) {
  try {
    console.log('🔍 Attempting to fetch group participants...');
    
    let participants = [];
    
    if (chat.participants && chat.participants.length > 0) {
      participants = chat.participants;
      console.log(`✅ Found ${participants.length} participants via chat.participants`);
    } 
    else if (chat.groupMetadata && chat.groupMetadata.participants) {
      participants = chat.groupMetadata.participants;
      console.log(`✅ Found ${participants.length} participants via groupMetadata`);
    } 
    else {
      console.log('🔄 Refreshing chat data...');
      const refreshedChat = await chat.fetch();
      participants = refreshedChat.participants || [];
      console.log(`✅ Found ${participants.length} participants after refresh`);
    }

    if (!participants || participants.length === 0) {
      console.log('❌ No participants found after all attempts');
      return null;
    }

    // Carefully filter and validate participants
    const eligibleParticipants = participants.filter(participant => {
      // Check if participant has basic structure
      if (!participant || !participant.id || typeof participant.id !== 'object') {
        console.log('⚠️ Filtered participant with invalid structure');
        return false;
      }
      
      // Validate the ID format carefully
      if (!participant.id._serialized || typeof participant.id._serialized !== 'string') {
        console.log('⚠️ Filtered participant with invalid _serialized ID');
        return false;
      }
      
      // Ensure it's a regular user and has user property
      const isRegularUser = participant.id._serialized.includes('@c.us') && 
                           participant.id.user && 
                           typeof participant.id.user === 'string';
    ...
    console.log(`🎯 ${eligibleParticipants.length} eligible participants after filtering`);
    ...
    console.log('❌ Critical error in getRandomGiveawayWinner:', error);
    return null;
  }
}
      ...
      if (message.body === '.giveaway') {
        if (chat.isGroup) {
          try {
            ...
            if (winner && winner.id && winner.id.user) {
              ...
              await message.reply('❌ Could not find valid participants for the giveaway. The group data may be corrupted.');
            }
          } catch (error) {
            console.log('Error in giveaway command:', error);
            await message.reply(`❌ Giveaway failed: ${error.message}`);
          }
        } else {
      ...
      if (message.body === '.tagall') {
        if (chat.isGroup) {
          try {
            ...
            // Safe participant fetching with validation
            if (chat.participants && Array.isArray(chat.participants) && chat.participants.length > 0) {
              ...
              try {
                ...
                console.log('Failed to refresh chat:', fetchError);
                await message.reply('❌ Failed to access group members. Please try again later.');
                return;
              }
            }
            ...
            // Carefully build mentions with validation
            const mentions = [];
            let mentionText = '';
            let validCount = 0;
            
            for (const participant of participants) {
              // Validate each participant thoroughly
              if (participant && 
                  participant.id && 
                  typeof participant.id === 'object' &&
                  participant.id._serialized && 
                  typeof participant.id._serialized === 'string' &&
                  participant.id.user && 
                  typeof participant.id.user === 'string' &&
                  participant.id._serialized.includes('@c.us')) {
            ...
            console.log(`✅ Valid participants for tagging: ${validCount}`);
            
            if (mentions.length === 0) {
              await message.reply('❌ No valid participants to tag. All participant data appears corrupted.');
              return;
            }
            ...
            await message.reply(`❌ Tagging failed: ${error.message}. This may be a WhatsApp limitation with large groups.`);
          }
        } else {
          await message.reply('❌ This command only works in groups.');
        }
